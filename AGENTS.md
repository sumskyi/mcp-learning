...
- `npm test` — run tests via `tsx --test src/**/*.test.ts`.
...
- The project is for learning: tools can return mocked data instead of making real
  external API calls when that is the current learning scope.

---

## Global Constitution (Imported)

Source: https://raw.githubusercontent.com/sumskyi/the-way/refs/heads/master/README.md

# The Way

*This is the canonical, source-of-truth version. Translations: [Українською](README.uk.md).*

**The Way exists so a project never slides into chaos.**

This repository is the foundation a project's daily life runs on — not a
style guide bolted on top, but the thing standing between order and drift.
It exists to close these off, permanently:

- the moment nobody can say whose rule wins, so everyone just freelances;
- the rule that's dead in practice but still hangs on the wall — enforced
  by no one, removed by no one;
- a project quietly filling up with process, abstraction, and ceremony
  nobody asked for;
- the "it works" nobody can actually verify, until it doesn't;
- the doc or name that stopped describing reality long ago and now
  actively misleads;
- an executor that either obeys blindly or does whatever it wants — never
  the disciplined middle.

Solve these once, at the root, and a project stops sliding into chaos and
uncertainty — it inherits the answer instead of re-litigating it every time.

*Version 1.0.1 — see [CHANGELOG.md](CHANGELOG.md) for the version history.
